...
internals.authenticate = function (name, options, callback) {
    ...
    return function authenticate (request, next) {
        ...
        var backupNext = function (err) {

            if (err) {
                return request.reply.redirect(options.failureRedirect || internals.settings.failureRedirect || '/').send();
            }
            return request.reply.redirect(options.successRedirect || internals.settings.successRedirect || '/').send();
        }
