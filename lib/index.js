        ...
        return function authenticate(request, next) {
            ...
            if (!next) {
                next = function (err) {
                    ...
                    return request.reply(internals.responseFromRedirect(options.failureRedirect || settings.redirect || '/'));
                };
            }
...
internals.Scheme.prototype.authenticate = function (request, callback) {
    ...
    return callback(internals.responseFromRedirect(this.settings.urls.failureRedirect));
    
    // if (typeof this.settings.urls.failureRedirect == 'object') {
    //     if (this.settings.urls.failureRedirect.code) {
    //         return callback(new this.hapi.error.passThrough(this.settings.urls.failureRedirect.code, this.settings.urls.failureRedirect.message || 'error with failureRedirect', this.settings.urls.failureRedirect.type || 'application/json', { 'Cache-Control': 'no-cache' }));
    //     }
    //     else {
    //         return callback(new this.hapi.error.unauthorized('Invalid credentials supplied. Access denied.', ['Travelogue']));
    //     }
    // }
...
internals.responseFromRedirect = function (redirectObj) {

    if (typeof redirectObj == 'string') {
        return new Hapi.response.Redirection(redirectObj || '/');
    }
    else if (typeof redirectObj == 'object') {
        if (redirectObj.code) {
            return new Hapi.error.passThrough(redirectObj.code || 500, redirectObj.message || 'error with failureRedirect', redirectObj.type || 'application/json', redirectObj.headers || { 'Cache-Control': 'no-cache' });
        }
        else {
            return new Hapi.error.unauthorized(redirectObj.message || 'Invalid credentials supplied. Access denied.', ['Travelogue']);
        }
    }
    else {
        return new Hapi.response.Redirection('/');
    }
};
