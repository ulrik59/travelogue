...
internals.initialize = function () {
    ...
    return function (request, next) {
        ...
        var passportSession = request.session.get(passport._key);
        if (passportSession) {                    
            ...
            passportSession = {};
            request.session.set(passport._key, passportSession);
            request._passport.session = passportSession;
        }
        
        request.body = request.payload;                     // passport-local Compatibility 

        // Link the passport session key to yar's API

        request.session.__defineGetter__(passport._key, function () {
            
            return request.session.get(passport._key);
        });

        request.session.__defineSetter__(passport._key, function (value) {

            request.session.set(passport._key, value);
        });
        ...
        request.login = request.logIn = function (user, options, done) {
            ...
            if (session) {
                request._passport.instance.serializeUser(user, function(err, obj) {
                    ...
                    if (err) {
                        request[property] = null; return done(err);
                    }
        ...
        request.logout = request.logOut = function () {
