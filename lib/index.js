...
exports.register = function (plugin, options, next) {
    ...
    plugin.auth.strategy('passport', 'passport', settings.defaultMode);
            ...
            if (!next) {
                next = function (err) {
                    if (err && err.isBoom) {
                        ...
                        return reply(err || 'You are being redirected...').redirect(err ? (options.failureRedirect || settings.failureRedirect || '/')
    ...
    return function (server, options) {
        ...
        var scheme = {
            authenticate: function (request, reply) {
                ...
                return reply('You are being redirected...', { err: 'Failed' }).redirect(options.failureRedirect || settings.failureRedirect || '/');
            }
        }
        return scheme;
    }
