...
var Hapi = null;                        // Assigned during registration
...
exports.register = function (plugin, options, next) {
    ...
    var settings = internals.settings = Hapi.utils.applyToDefaults(internals.defaults, options);
...
internals.authenticate = function (name, options, callback) {
    ...
    return function authenticate (request, next) {
        ...
        var backupNext = internals.backupNextFactory(request, options);
        
        next = next || backupNext;
...
internals.backupNextFactory = function (request, options) {

    return function (err) {

        if (err) {
            return request.reply.redirect(options.failureRedirect || internals.settings.failureRedirect || '/');
        }
        return request.reply.redirect(options.successRedirect || internals.settings.successRedirect || '/');
    };
};
