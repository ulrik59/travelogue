                    ...
                    if (err && err.isBoom) {
                        ...
                        if (settings.apiMode || options.apiMode) {
                            return err ? reply(Hapi.error.unauthorized('Unauthorized'))
                                        : reply().code(204).header('Location', options.successRedirect || settings.urls.successRedirect || '/');
                        } else {
                            return reply(err || 'You are being redirected...').redirect(err ? (options.failureRedirect || settings.urls.failureRedirect || '/')
                                                                                            : (options.successRedirect || settings.urls.successRedirect || '/'));
                        }
                    }
                };
            }
